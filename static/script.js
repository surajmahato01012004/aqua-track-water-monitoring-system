...
wqiForm.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        let chartColor;
        if (result.wqi <= 25) chartColor = '#28a745';          // green
        else if (result.wqi <= 50) chartColor = '#0d6efd';      // blue
        else if (result.wqi <= 75) chartColor = '#ffc107';      // yellow
        else if (result.wqi <= 100) chartColor = '#dc3545';     // red
        else chartColor = '#343a40';                            // black
